import { Github, GitCommit, Star, Users, GitFork, AlertCircle, Scale, HardDrive, Hash, ExternalLink, ShieldCheck, Cpu, Box } from "lucide-preact";
...
export default function ProjectDetail({ projects, owner, repo }: ProjectDetailProps) {
  ...
  if (!project) {
    return (
      <main class="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <a href={`${import.meta.env.BASE_URL}#/`} class="text-vibe-accent hover:underline mt-4 inline-block">
          Back to list
        </a>
      </main>
    );
  ...
  const langs = project.languages && Object.keys(project.languages).length > 0
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6">
      ...
      <div class="rounded-2xl bg-vibe-elevated border border-vibe-border overflow-hidden">
        ...
        <div class="p-6 sm:p-8 border-b border-vibe-border">
          ...
          {project.description && (
            <p class="mt-6 text-lg text-vibe-muted leading-relaxed max-w-3xl">{project.description}</p>
          )}
          ...
          <div class="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              ...
              <div class="font-mono font-bold text-vibe-fg">{project.stars ?? 0}</div>
            </div>
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              <div class="text-xs text-vibe-muted flex items-center gap-1 mb-1">
                <GitCommit size={12} /> Commits
              </div>
              <div class="font-mono font-bold text-vibe-fg">{project.commits ?? 0}</div>
            </div>
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              <div class="text-xs text-vibe-muted flex items-center gap-1 mb-1">
                <Users size={12} /> Contribs
              </div>
              <div class="font-mono font-bold text-vibe-fg">{project.contributors ?? 0}</div>
            </div>
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              <div class="text-xs text-vibe-muted flex items-center gap-1 mb-1">
                <GitFork size={12} /> Forks
              </div>
              <div class="font-mono font-bold text-vibe-fg">{project.forks ?? 0}</div>
            </div>
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              <div class="text-xs text-vibe-muted flex items-center gap-1 mb-1">
                <AlertCircle size={12} /> Issues
              </div>
              <div class="font-mono font-bold text-vibe-fg">{project.open_issues ?? 0}</div>
            </div>
            <div class="p-3 rounded-xl bg-vibe-bg border border-vibe-border">
              <div class="text-xs text-vibe-muted flex items-center gap-1 mb-1">
                <ShieldCheck size={12} /> Vulns
              </div>
              <div class="font-mono font-bold text-red-500">{project.vulnerableDependencies?.length ?? 0}</div>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 sm:p-8 grid md:grid-cols-2 gap-8">
          ...
          <div class="space-y-8">
            <section>
              ...
              <div class="space-y-4">
                  ...
                  <div class="flex flex-wrap gap-x-4 gap-y-2">
                    {langs.map(([lang, bytes]) => (
                      <div key={lang} class="flex items-center gap-1.5 text-xs">
                        ...
                        <span class="text-vibe-muted">{((bytes / totalBytes) * 100).toFixed(1)}%</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div class="flex flex-wrap gap-2 text-xs">
                  ...
                  {project.size != null && (
                    <span class="px-2.5 py-1 rounded-lg bg-vibe-bg border border-vibe-border text-vibe-fg flex items-center gap-1.5">
                      <HardDrive size={12} /> {(project.size / 1024).toFixed(1)} MB
                    </span>
                  )}
            ...
            <section>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="p-4 rounded-xl bg-vibe-bg border border-vibe-border space-y-3">
                  <div class="text-xs font-semibold text-vibe-muted uppercase">Package Manager</div>
                  {project.packageManager ? (
                    ...
                    <div class="text-xs text-vibe-muted italic">None detected</div>
                  )}
                </div>
                <div class="p-4 rounded-xl bg-vibe-bg border border-vibe-border space-y-3">
                  <div class="text-xs font-semibold text-vibe-muted uppercase">AI Tools Used</div>
            ...
            {project.topics && project.topics.length > 0 && (
              <section>
                <h2 class="text-sm font-bold uppercase tracking-wider text-vibe-muted mb-3">Topics</h2>
                <div class="flex flex-wrap gap-2">
                  {project.topics.map((t) => (
                    <span key={t} class="text-[10px] px-3 py-1 rounded-full bg-vibe-bg border border-vibe-border text-vibe-muted">
                      #{t}
                    </span>
                  ))}
                </div>
          ...
          <div class="space-y-8">
            {project.owner && (
              <section>
                <h2 class="text-sm font-bold uppercase tracking-wider text-vibe-muted mb-4">Author</h2>
                <div class="p-5 rounded-2xl bg-vibe-bg border border-vibe-border space-y-4">
                  <div class="flex items-center gap-4">
                    <a href={project.owner.url} target="_blank" rel="noopener noreferrer" class="shrink-0">
                      <img
                    ...
                    <div class="min-w-0">
                      ...
                      <div class="flex flex-wrap gap-x-4 text-[10px] text-vibe-muted">
                        {project.owner.created_at && (
                          <span>Joined: {formatAge(project.owner.created_at)} ago</span>
                        )}
                      </div>
                    </div>
                  </div>
                    ...
                    <>
                      ...
                      <div class="flex items-center gap-6 pt-2">
                        <div class="text-center">
                          <div class="text-xs font-bold text-vibe-fg">{project.owner.followers ?? 0}</div>
                          <div class="text-[10px] text-vibe-muted uppercase tracking-wider">Followers</div>
                        </div>
                        <div class="text-center">
                          <div class="text-xs font-bold text-vibe-fg">{project.owner.following ?? 0}</div>
                          <div class="text-[10px] text-vibe-muted uppercase tracking-wider">Following</div>
                        </div>
                        <div class="text-center">
                          <div class="text-xs font-bold text-vibe-fg">{project.owner.public_repos ?? 0}</div>
                          <div class="text-[10px] text-vibe-muted uppercase tracking-wider">Repos</div>
                        </div>
                      </div>
                    </>
                  )}
            ...
            {project.contributorDetails && project.contributorDetails.length > 0 && (
              <section>
                <h2 class="text-sm font-bold uppercase tracking-wider text-vibe-muted mb-4">Top Contributors</h2>
                <div class="grid grid-cols-2 gap-3">
                  {project.contributorDetails.slice(0, 10).map((c) => (
                    ...
                    >
                      ...
                      />
                      <div class="min-w-0">
                        <div class="text-xs font-bold text-vibe-fg truncate group-hover:text-vibe-accent transition">{c.login}</div>
                        {c.created_at && <div class="text-[10px] text-vibe-muted truncate">{formatAge(c.created_at)}</div>}
                      </div>
                    </a>
                  ))}
                </div>
            ...
            {project.proof_sources && project.proof_sources.length > 0 && (
              <section>
                <h2 class="text-sm font-bold uppercase tracking-wider text-vibe-muted mb-4">Proof & Sources</h2>
                <div class="space-y-2">
                  {project.proof_sources.map((s, i) => (
                    ...
                    >
                      ...
                      <ExternalLink size={14} class="text-vibe-muted group-hover:text-vibe-accent transition" />
                    </a>
                  ))}
                </div>
              </section>
          ...
          <div class="p-6 sm:p-8 bg-vibe-bg border-t border-vibe-border">
            <div class="flex items-center justify-between mb-6">
              <h2 class="text-lg font-bold text-vibe-fg flex items-center gap-2">
                ...
                Security Audit Findings ({project.vulnerableDependencies!.length})
              </h2>
              ...
              <table class="w-full text-sm text-left border-collapse">
                <thead>
                  <tr class="text-vibe-muted border-b border-vibe-border">
                    <th class="pb-3 pr-4 font-semibold uppercase tracking-wider text-[10px]">Severity</th>
                    <th class="pb-3 pr-4 font-semibold uppercase tracking-wider text-[10px]">Target</th>
                    <th class="pb-3 pr-4 font-semibold uppercase tracking-wider text-[10px]">Identifier</th>
                    <th class="pb-3 font-semibold uppercase tracking-wider text-[10px]">Description / Recommendation</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-vibe-border/50">
                  {project.vulnerabilities?.map((v, i) => (
                    <tr key={i} class="group hover:bg-vibe-elevated/50 transition-colors">
                      <td class="py-4 pr-4 align-top">
                        <span class={`px-2 py-0.5 rounded text-[10px] font-bold ${
                          v.severity === 'CRITICAL' ? 'bg-red-500/20 text-red-600' :
                          v.severity === 'HIGH' ? 'bg-orange-500/20 text-orange-600' :
                          v.severity === 'MEDIUM' ? 'bg-amber-500/20 text-amber-600' :
                      ...
                      <td class="py-4 pr-4 align-top font-mono text-xs text-vibe-fg">
                        {v.type === "secret" ? "Repository" : `${v.pkg}@${v.version}`}
                      </td>
                      <td class="py-4 pr-4 align-top">
                        <span class="font-mono text-xs text-vibe-muted group-hover:text-vibe-accent transition">{v.cve}</span>
                      </td>
                      <td class="py-4 align-top">
                        <div class="text-vibe-fg font-medium mb-1">
                          {v.type === "secret" && <span class="text-red-500 font-bold mr-1">[SECRET]</span>}
                          {v.title}
                        </div>
                        {v.fixedVersion && (
