...
function LangBar({ languages }: { languages: Record<string, number> }) {
  ...
  return (
    <div
      class="absolute top-0 left-0 w-1.5 sm:w-2 rounded-l-xl rounded-r-none h-full flex flex-col overflow-hidden"
      aria-hidden
    >
...
export default function ProjectCard({ project }: ProjectCardProps) {
  const hasMultiLang = project.languages && Object.keys(project.languages).length > 0;
  const singleLangColor = project.language && !hasMultiLang
  ...
  return (
    <div
      class="group relative block p-4 sm:p-5 rounded-xl bg-vibe-elevated border border-vibe-border hover:border-vibe-muted/60 transition-all touch-manipulation min-h-[160px] flex flex-col"
    >
      <a 
      ...
      {hasMultiLang ? (
        ...
        <div
          class="absolute top-0 left-0 w-1 sm:w-1.5 rounded-l-xl rounded-r-none h-full"
          style={{ backgroundColor: singleLangColor }}
          aria-hidden
        />
      ) : null}
      <div class={`${hasMultiLang || singleLangColor ? "pl-4 sm:pl-5" : ""} relative z-10 flex-grow`}>
        <div class="flex items-start justify-between gap-3 mb-2">
          ...
          <span class="shrink-0 flex items-center gap-2 sm:gap-3 text-vibe-muted text-sm relative z-20">
            {project.created_at && (
              <span class="text-[10px] bg-vibe-muted/10 px-1.5 py-0.5 rounded" title="Repository Age">
            ...
            {project.commits != null && project.commits > 0 && (
              <span class="flex items-center gap-1" title="Commits">
                ...
                {project.commits >= 1000 ? `${(project.commits / 1000).toFixed(1)}k` : project.commits}
              </span>
            )}
            ...
            {project.stars != null && project.stars > 0 && (
              <span class="flex items-center gap-1">
                <Star size={14} strokeWidth={2} class="text-amber-500 dark:text-amber-400" />
                {project.stars}
              </span>
            )}
            <a
            ...
            >
              <Github size={14} strokeWidth={2} class="opacity-60 group-hover:opacity-100 transition" />
            </a>
          </span>
        </div>
        {project.description && (
        ...
        <div class="mt-3 flex flex-wrap items-start justify-between gap-2">
          ...
          <div class="flex flex-wrap gap-2 items-center">
            ...
            {project.packageManager && (
              <span 
                class="text-xs px-2.5 py-1 rounded-lg bg-orange-500/20 text-orange-600 dark:text-orange-400 cursor-help"
            ...
            {(project.aiTools || []).map((t) => (
              <span 
                key={t.name} 
                class="text-xs px-2.5 py-1 rounded-lg bg-violet-500/20 text-violet-600 dark:text-violet-400 cursor-help"
            ...
            {(project.vulnerableDependencies || []).length > 0 && (
              <span class="relative group/vuln z-30">
                  ...
                  <div class="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 z-40 hidden group-hover/vuln:block w-72 max-w-[calc(100vw-2rem)] opacity-0 group-hover/vuln:opacity-100 transition-opacity">
                    <div class="bg-vibe-elevated border border-vibe-border rounded-lg shadow-lg p-3 text-left">
                      <div class="flex items-center justify-between mb-2">
                        <p class="text-xs font-medium text-vibe-fg">Vulnerabilities</p>
                        <span class="text-[10px] text-vibe-muted italic">Scanned using Trivy</span>
                      </div>
                      <ul class="space-y-2 max-h-48 overflow-y-auto">
                        {project.vulnerabilities.map((v) => (
                          <li key={`${v.pkg}-${v.cve}-${v.type}`} class="text-xs">
                            {v.type === "secret" ? (
                              <>
                                <span class="font-bold text-red-600 dark:text-red-500">[SECRET]</span>
                                <span class="text-vibe-fg ml-1">{v.title}</span>
                                <span class="text-vibe-muted ml-1">({v.cve})</span>
                              </>
                            ) : (
                              <>
                                <span class="font-mono text-vibe-fg">{v.pkg}@{v.version}</span>
                                <span class="text-red-500 dark:text-red-400 ml-1">({v.cve})</span>
                              </>
                            )}
                            <span class="text-vibe-muted ml-1">{v.severity}</span>
                            {v.title && v.type !== "secret" && <p class="mt-0.5 text-vibe-muted line-clamp-2">{v.title}</p>}
                            {v.fixedVersion && (
                              <p class="text-emerald-600 dark:text-emerald-400 text-[10px]">Fixed: {v.fixedVersion}</p>
                            )}
                          </li>
                        ))}
                      </ul>
          ...
          {project.lastUpdated && (
            <p class="text-[10px] text-vibe-muted">
              Updated: {new Date(project.lastUpdated).toLocaleDateString(undefined, { dateStyle: "short" })}
          ...
          {project.owner && (
            <a
              ...
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              class="flex flex-col items-end gap-0.5 text-vibe-muted hover:text-vibe-fg transition shrink-0"
